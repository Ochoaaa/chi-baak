...
async function upload(
  ...
  if (!TYPES.includes(f.type) || f.size > 5_000_000) {
    throw new Error(
...
export async function saveGalleryItem(fd: FormData) {
  ...
  if (!id && (!before || !after)) {
    throw new Error(
      "Sube la imagen de antes y la de después."
    );
  }
  ...
  if (id) {
    ...
    if (before) {
      row.before_image_url = before;

      const oldPath = pathOf(
...
export async function deleteGalleryItem(fd: FormData) {
  ...
  const paths = [
    pathOf(old?.before_image_url),
    pathOf(old?.after_image_url),
